...
start() {
    check_running
    echo "starting $CMD ..."
	# 将运行时的conf目录放到path中，用于查找setting.py
	export PYTHONPATH=$PYTHONPATH:$EXECUTEDIR/conf
	export DJANGO_SETTINGS_MODULE="django_settings"
    "$BASEDIR"/lib/"$CMD" -d "$EXECUTEDIR" 2>"$EXECUTEDIR"/log/"$NAME".err >"$EXECUTEDIR"/log/"$NAME".out &
    PID=$!
